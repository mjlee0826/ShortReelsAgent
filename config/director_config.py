...
from config.env_utils import read_int_env
...
DIRECTOR_AGENTIC_MAX_STEPS = read_int_env("DIRECTOR_AGENTIC_MAX_STEPS", 24)

# submit_blueprint 後 Critic 驗證不過 → 把錯誤當 tool_result 餵回同一對話就地修的最大重試次數
# (沿用舊 ReflectionState 的 3 次;達上限即輸出當前草稿,行為與舊一致)。
DIRECTOR_MAX_CRITIC_RETRIES = read_int_env("DIRECTOR_MAX_CRITIC_RETRIES", 3)

# get_fields 單次最多投影幾個素材 id(杜絕模型一次把整庫所有重欄位拉出來,違背漸進精讀初衷)。
DIRECTOR_GET_FIELDS_MAX_IDS = read_int_env("DIRECTOR_GET_FIELDS_MAX_IDS", 60)

# view_raw 單一影片最多抓幾張幀;像素 token 昂貴,故封頂。
DIRECTOR_VIEW_RAW_MAX_FRAMES = read_int_env("DIRECTOR_VIEW_RAW_MAX_FRAMES", 4)

# view_raw 單次呼叫（可含多個素材）的總幀數上限：多素材批次看省 API 往返，但總像素 token 需封頂,
# 避免一次看整庫把 context 撐爆;超出上限的素材回覆提示分批再看。
DIRECTOR_VIEW_RAW_MAX_TOTAL_FRAMES = read_int_env("DIRECTOR_VIEW_RAW_MAX_TOTAL_FRAMES", 12)

# view_template 單次最多抓幾張範本幀。範本切點可能很多,但每張幀都是昂貴的像素 token,故另封一個
# 上限(與看單一素材的 view_raw 分開計):值略高於 view_raw,讓導演一次看清範本的鏡頭序列 / 節奏。
DIRECTOR_VIEW_RAW_TEMPLATE_MAX_FRAMES = read_int_env("DIRECTOR_VIEW_RAW_TEMPLATE_MAX_FRAMES", 6)

# 單張圖片(照片)長邊降到此像素再 base64。「確認構圖 / 主體與 metadata 相符」768 已足夠
# (原 1080 對此用途偏奢侈,token 約省 ~40%);細節敏感情境可用 env 調回 1080。
DIRECTOR_VIEW_RAW_DOWNSCALE_PX = read_int_env("DIRECTOR_VIEW_RAW_DOWNSCALE_PX", 768)

# 影片幀 montage 拼格的每格長邊像素:多幀拼成一張圖,每格 512 對「判斷構圖 / 主體 / 節奏」足夠,
# 相比逐幀單張大圖約 3–4× 省像素 token;要更銳利可 env 調 640/768(token 同步上升)。
DIRECTOR_VIEW_RAW_MONTAGE_CELL_PX = read_int_env("DIRECTOR_VIEW_RAW_MONTAGE_CELL_PX", 512)
...
DIRECTOR_TASK_BUDGET_TOKENS = read_int_env("DIRECTOR_TASK_BUDGET_TOKENS", 0)
