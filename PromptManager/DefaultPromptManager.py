...
class DefaultPromptManager(BasePromptManager):
    ...
    def get_timecoded_action_index_prompt(self) -> str:
        return (
            ...
            "2. 請根據左上角的實際秒數，將影片拆解為數個『連續的動作區塊』。\n"
            "3. 描述每個區塊的動作時，請專注於人物肢體、運鏡與動態，忽略背景與服裝。\n"
            "4. 同時給出整支影片的『全局攝影評論』。\n\n"
            "【⚠️ 嚴厲警告 (防幻覺)】\n"
            "- 絕對不可照抄下方 JSON 格式範例的文字！你必須根據真實觀看的內容生成專屬的分析。\n"
            "- 動作區塊的 end_time 絕對不可以超過這支影片的實際總長度！\n\n"
            "【嚴格格式要求】請直接輸出 JSON，不要包含 markdown 標記。時間請使用小數(float)。請將下方的空字串與 0.0 替換為你的真實分析：\n"
            "{\n"
            "  \"cinematic_critique\": \"\",\n"
            "  \"action_index\": [\n"
            "    {\n"
