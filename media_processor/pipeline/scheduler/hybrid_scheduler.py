...
class HybridScheduler:
    ...
    def run(
        ...
        with ThreadPoolExecutor(
            ...
            # copy_context:把父緒(run_phase1 / producer)的成本帳本帶進 driver 緒;
            # 迴圈變數已叫 ctx(AssetContext),故用 copy_context().run 內聯避免名稱衝突
            futures = [
                driver_pool.submit(contextvars.copy_context().run, self._drive_one, ctx, tracker)
                for ctx in contexts
            ]
