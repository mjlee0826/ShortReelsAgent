...
> **落地現況**:image 端已在 G1 單張 MUSIQ tech_score 之後、由 G2 `RejectFilterStage` 短路(`technical_score < TECHNICAL_SCORE_FILTER_THRESHOLD=40`),
> reject 後 Pipeline 自動跳過 G3(含 Qwen)/G4。**「跑完 MUSIQ batch 後才 reject」**要等 Week 3a 接 `BatchCollector`;影片端 Early Rejection 留 Week 2c。
...
- ✅ **Week 2a 已完成**(commit `03b98bb`):ExecutorRegistry + ModelPoolRegistry + Pipeline + StageGroup + HybridScheduler + Builder/Runner + LegacyStage;替換 `director_service.py` 呼叫端
- ✅ **Week 2b 已完成**(2026-06-02):image Stage 拆解(10 Stage + `ImageWork` dataclass)+ Early Rejection 短路;**semantic 併入 G3 平行群**(不獨立成 G4,避免 qwen 空等 CPU stage);`USE_LEGACY_IMAGE_PIPELINE` 旗標可 A/B 回退。端到端實機 A/B 待跑
- ⬜ **Week 2c**:video Stage 拆解 + StageGroup 編排
...
| `media_processor/pipeline/`(框架) | ✅ Week 2a 新增 | Pipeline / StageGroup / Executor / Scheduler / Builder / Runner 模組樹 |
...
| `config/pipeline_config.py` | ✅ Week 2a 新增 / Week 2b 加旗標 | 併發/timeout 常數;Week 2b 加 `USE_LEGACY_IMAGE_PIPELINE`(A/B 回退)|
...
*文件最後更新:2026-06-02(Week 2b image Stage 拆解完成)*
