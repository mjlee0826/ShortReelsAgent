...
| `model/whisper_model_manager.py` | ✅ Week 1 + Week 3a 接入 | Week 1 新增 `transcribe_batch()`;Week 3a 接入 BatchCollector |
...
| `media_processor/pipeline/progress/` | ✅ Week 1 新增 | ProgressObserver/Tracker/Event + PrintObserver(套件:events/observer/tracker)|
...
| `config/pipeline_config.py` | ✅ Week 2a 新增 / 2b / 2c / 3a 加旗標 | 併發/timeout 常數;2b 加 `USE_LEGACY_IMAGE_PIPELINE`;2c 加 `USE_LEGACY_VIDEO_PIPELINE`;3a 加 4 個 `*_BATCH_ENABLED` + `MAX_ASSETS_PARALLEL` 改預設 8 |
...
*文件最後更新:2026-06-03(Week 3b 雙 GPU Pool + GpuCapacityManager + BudgetGate 完成;§15.1 標記完成、§16 補錄 Week 3b 接觸檔案;全模型接 pool + Qwen 兩層優先、oom_resilient、borrow 即時 VRAM 重檢等設計決策)*
