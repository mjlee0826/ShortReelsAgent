...
- ⬜ **Week 3c**:ProgressTracker + WebSocket 端到端接前端
...
| 檔案 | 改動類型 | 說明 |
|---|---|---|
| `backend/services/director_service.py` | ✅ Week 2a 已改 | 呼叫端替換,L92 序列迴圈改為 `PipelineRunner.run(...)` |
...
| `media_processor/pipeline/progress/` | ✅ Week 1 新增 / 2026-06-04~05 擴充 | ProgressObserver/Tracker/Event + PrintObserver(events/observer/tracker);**新增** `watchdog.py`(StallWatchdog + faulthandler dump)、`system_health.py`(SystemHealthProbe) |
...
| `media_processor/pipeline/`(框架) | ✅ Week 2a 新增 / Week 2c 升級 | Pipeline / Executor / Scheduler / Builder / Runner 模組樹;Week 2c `StageNode` DAG 取代 StageGroup(`stage_group.py` 已移除,新增 `node.py`) |
...
*文件最後更新:2026-06-05(模型換代與資源歸屬修正:Qwen **8B→4B**、Whisper 改 **faster-whisper turbo**、
