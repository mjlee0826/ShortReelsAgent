...
| 招式 | 加速 | VRAM 變化 | 機制 |
|---|---|---|---|
| 4-bit AWQ 量化 | 1.5–2x | **-50%**(10GB → 5GB) | 換 `cyankiwi/Qwen3-VL-8B-Instruct-AWQ-4bit` model id(見下方註) |
...
> **⚠️ AWQ 來源實況(Week 1 落地修正)**:Qwen **官方未釋出** `Qwen3-VL-8B-Instruct-AWQ`。
> 改用社群版 `cyankiwi/Qwen3-VL-8B-Instruct-AWQ-4bit`(月下載 124k、Apache-2.0)。
> 注意三點:(1) 它用 **llm-compressor / compressed-tensors 格式**(非 autoawq),需 `pip install compressed-tensors`;
> (2) **processor 仍從官方 base `Qwen/Qwen3-VL-8B-Instruct` 載**;(3) 載入用 `dtype="auto"` 不可再傳 `BitsAndBytesConfig`。
> 連帶:`compressed-tensors>=0.16` 要 `torch>=2.10`,故環境 torch 定在 2.10、flash-attn 用 torch2.10 預編譯 wheel。
> 詳見 `implementation_roadmap.md` 第 3 章。
...
- ✅ Qwen AWQ(cyankiwi 4-bit)+ Flash Attention 2 換上,FA2 實機驗證生效
...
- 舊版迴圈仍可用,`QWEN_USE_AWQ=false` 可切回 8-bit 測品質差異
...
| `model/qwen_model_manager.py` | ✅ Week 1 已改 | cyankiwi AWQ + FA2 fallback + device_map 鎖定 self.device |
...
| `config/media_processor_config.py` | ✅ Week 1 已改 | batch size / GPU buffer / AWQ 切換常數 |
| `config/model_config.py` | ✅ Week 1 已改 | Qwen AWQ/legacy/processor id + FA2 flag |
