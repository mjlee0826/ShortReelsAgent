...
| 招式 | 加速 | VRAM 變化 | 機制 |
|---|---|---|---|
| 4-bit AWQ 量化 | 1.5–2x | **-50%**(10GB → 5GB) | 換 `Qwen3-VL-8B-Instruct-AWQ` model id |
...
### 4.5 GPU-level Semaphore(修 OOM bug)
...
**解法**:在 `BaseModelManager` 加一層全域 `GPU_SEMAPHORES`,每張 GPU 一個 Semaphore(1)。鎖的取得順序:
...
| 優化 | 加速 | 來源 |
|---|---|---|
| AWQ + Flash Attn | 3x | 同一個 Qwen 跑得更快 |
...
- 用途:AWQ 模型升級、之前失敗想重試、想重新審視 metadata
...
| 風險 | 緩解 |
|---|---|
| AWQ 量化品質下降 | Week 1 先單獨驗證 caption / mood / scene_tags 與 8-bit 版本的差異,不通過則退回 |
...
**Week 1**:Layer 1 模型層獨立優化(低風險,大紅利)
- Qwen AWQ + Flash Attention 換上 → 直接享受 3x Qwen 加速
- GPU Semaphore 加入 → 修 OOM bug
- 此週結束前舊版迴圈仍可用,可測 Qwen 換版本的品質差異
...
1. **品質回歸**:同一組 asset 用舊版與新版各跑一次,比對 `phase1_assets_metadata.json` 欄位差異;AWQ 帶來的 caption 文字微差可接受,但 `technical_score` / `subject_bbox` / `scene_cuts` 等結構化欄位必須一致
...
| 檔案 | 改動類型 | 說明 |
|---|---|---|
| `backend/services/director_service.py` | 修改 | 呼叫端替換,L92 序列迴圈改為 `PipelineRunner.run(...)` |
| `model/base_model_manager.py` | 修改 | 加入 GPU Semaphore 層 |
| `model/qwen_model_manager.py` | 修改 | AWQ + Flash Attention 載入設定 |
| `model/musiq_model_manager.py` | 修改 | 新增 `score_batch()` 方法 |
| `model/laion_model_manager.py` | 修改 | 新增 `score_batch()` 方法 |
| `model/whisper_model_manager.py` | 修改 | 新增 `transcribe_batch()` 方法 |
| `model/model_pool.py` | 不修改 | 直接使用 |
| `media_processor/abstract_image_processor.py` | 邏輯沿用 | 內部方法搬到對應 Stage |
| `media_processor/abstract_video_processor.py` | 邏輯沿用 | 內部方法搬到對應 Stage |
| `media_processor/pipeline/` | 新增 | Stage / Pipeline / Executor / Scheduler / Progress 模組樹 |
| `config/pipeline_config.py` | 新增 | 集中所有併發/timeout 常數 |
| `backend/api/progress.py` | 新增 | WebSocket 進度端點 |
| `backend/api/workspaces.py` | 新增 | Google Drive workspace 管理 API |
| `ingestion_engine/` | 新增 | Layer 0 攝取模組(rclone + Google Drive polling) |
| `frontend/src/pages/AssetListPage.tsx` | 新增 | Layer 5 前端 UI |
