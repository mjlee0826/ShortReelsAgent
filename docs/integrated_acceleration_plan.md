...
│  IngestionPipeline: 公開資料夾 URL → 一個 project         │
│  Google Drive API v3 + 共用 API key + polling             │
└──────────────────────────────────────────────────────────┘
```
...
> **✅ 2026-06-05 已實作並改版（以本區為準,覆寫下方 §3.2 / §3.4 / §3.6 的 rclone / OAuth / Workspace 設計）**:
> 攝取改走 **公開資料夾 URL + 一把全站共用 Google Drive API key**,徹底拿掉 rclone 與 OAuth。
> 1. **授權**:資料夾設「知道連結的人可檢視」(anyone-with-link),後端以 `GOOGLE_API_KEY`(Cloud Console
>    點一下、無同意畫面、非 per-user)用 Drive API v3 列檔／`alt=media` 下載。**零 rclone、零 OAuth、零 per-user token**。
>    (覆寫 §3.4)
> 2. **模型壓平**:原「Workspace 根資料夾 → 子資料夾各自成 project」改為 **一個資料夾 URL = 一個 project**
>    (不列舉子資料夾,資料夾內檔案即素材)。(覆寫 §3.2)
> 3. **狀態落地**:不另建註冊檔;雲端來源與同步狀態折進該 project 的 `project_meta.json`
>    (`source=gdrive` / `drive_folder_id` / `source_url` / `phase1_status` / `sync_status` /
>    `last_asset_signature` / `last_synced_at` / `last_sync_error`)。poller 改掃所有 project 找 `source=gdrive` 者同步。
> 4. **同步策略**(取代 §3.5 的 archived 模型):素材簽章(檔名+大小)變動才增量下載 + 重跑 Phase 1;
>    資料夾轉私人／key 無權 → 該 project `sync_status=paused_auth_error`(其他不受影響),需手動 sync 喚醒。
> 5. **API**(取代 §3.5 的 `/api/workspaces/{id}/sync`):`POST /api/projects/from-drive`(貼 URL 建 project + 背景首同步)、
>    `POST /api/projects/{name}/sync`(手動)、`GET /api/projects` 露出同步欄位。
> 6. **為什麼 Drive API 而非 rclone**(覆寫 §3.6):rclone 需 per-user `rclone config` 互動授權、無法自助;
>    公開資料夾 + API key 讓使用者只要貼連結即可,單一雲端來源也不需 rclone 的多雲端統一層。
> 7. **檔案**:`ingestion_engine/`(`exceptions` / `cloud_storage_adapter`(ABC) / `public_drive_api_adapter`
>    / `cloud_ingestion_service` / `poller` / `models`)+ `config/ingestion_config.py` + `backend/api/projects.py`
>    新端點;新依賴 `httpx`,不需 rclone。詳見 `implementation_roadmap.md §10`。

### 3.1 設計目標

把「上傳到雲端 → 自動處理」做成天然體驗,user 拍照上傳就完成 Phase 1,不需手動上傳到後端。

**MVP 範圍**:只支援 Google Drive。其他雲端(Dropbox/S3/HTTP)列入未來擴展。架構不綁定來源,後續新增其他雲端只需實作一個新的 `CloudStorageAdapter`(同 `parse_source`/`list_files`/`download_folder` 介面),不動 Ingestion 主邏輯。
...
> **⚠️ 2026-06-05 模型換代(以本區為準,覆寫下方 §4.1 / §4.2 的舊型號與數字)**:
> 1. **Qwen 8B → 4B**(`Qwen/Qwen3-VL-4B-Instruct`):semantic stage 的 Qwen 只做 SIMPLE / 全局分析
>    (COMPLEX 與時間碼走 Gemini),任務單純、4B 已足夠且推論更快更省。常駐 VRAM(4-bit)由 8B 的
>    ~6.4GB 降到 **~3.5GB**(估值,待實機 `reset_peak_memory_stats` 校準);下方「18GB→6.4GB」「5s→0.6–1s」
>    均為 8B 時數字,換 4B 後更低。量化路徑(bnb 4-bit NF4)與 FA2 不變。
> 2. **Whisper:transformers large-v3 → faster-whisper(CTranslate2)large-v3-turbo**:turbo decoder 僅 4 層
>    (large-v3 為 32 層)、多語保留、速度數倍;CT2 量化 kernel(CUDA float16 / CPU int8)再加速並降 VRAM
>    (~3GB → ~1.6GB)。`info.language` 讓下游 `spoken_language` **真正生效**(舊 HF 路徑此欄一直為空)。
>    **注意**:faster-whisper 無「多檔一次 forward」介面,`transcribe_batch` 改以單借出實例**循序**轉錄
>    (維持 BatchCollector 的 list→list 等長契約),故下方 §4.2「Whisper batch 4 = 5x」**不再適用**。
...
| ~~Whisper~~ | ~~8s/檔~~ | ~~1.5s/檔(batch 4)~~ | ~~5x~~（**2026-06-05 換 faster-whisper 後不再多檔合批,見 §4 換代塊**）|

Saliency、VAD、Gemini 底層不支援 batch,維持單張(Whisper 自 2026-06-05 起亦改循序,見上)。
...
- 每個支援 batch 的 Stage 透過 `BatchCollectorRegistry.get(spec, batch_fn)` 取得**跨 asset 共享的
  `BatchCollector` 單例**(key 為 `"musiq"` / `"laion"` 等,class-level Registry + 雙重檢查鎖,
  對齊 `BaseModelManager._GPU_GATES`)。Stage 每個 asset 重新 new,**Collector 不在 Stage 內持有**,
  否則每 asset 一個 collector、永遠合不了批。
- Stage 介面不變,driver thread 完全無感
- 兩個關鍵參數:`batch_size`(越大吞吐越好、延遲增加)、`timeout_ms`(避免末尾 asset 卡死)
- **有效批量由上游併發決定**(timeout 驅動,非 batch_size 驅動):圖片 `tech`(MUSIQ)inline 在 driver
  thread 跑 → 有效批量 ≤ `MAX_ASSETS_PARALLEL`;GPU-pool 類 stage → 有效批量 ≤ GPU pool size。
  `batch_size` 是上限,典型預設併發下實際批量約 4–8。要量到 batch=16 的吞吐,需把
  `MAX_ASSETS_PARALLEL` 調到 16(env 可調)。
- 這個機制等同於 **NVIDIA Triton Inference Server / vLLM 的 dynamic batching**,只是縮小到 Stage 內部

### 4.4 Early Rejection 短路(省呼叫次數)✅ Week 2b 已實作(image)

跑完 MUSIQ 後,`technical_score < 40` 的 asset 直接標記 rejected,**完全不進入後續 Saliency / Aes / Qwen / Audio 等昂貴 stage**。

實測 30–50% 素材會被 reject(用戶丟一堆模糊照很正常),這部分等於**白送的加速**。

> **落地現況**:image 端 `TechScoreStage → RejectFilterStage`(`technical_score < TECHNICAL_SCORE_FILTER_THRESHOLD=40`)
> 短路,reject 後 Pipeline 自動跳過後續平行群(Saliency/Aes/CV/Face/Exif/Semantic)。
> ✅ **Week 3a**:MUSIQ 已接 `BatchCollector`(key `"musiq"`,保比例+padding 前處理),
> 「跑完 MUSIQ batch 後才 reject」已生效;影片端 Simple Pipeline 同樣有 `tech → reject_filter` 早 reject gate。
...
> **✅ Week 3b 已實作 / ⚠️ 2026-06-05 更正模型歸屬**:`ModelPoolRegistry.warm_up` —— **GPU capacity pool**:
> Qwen / Whisper / LAION / MUSIQ / AudioEnv;**CPU pool**(`_warm_up_auxiliary`,三池同構、各放多份獨立 instance
> 真平行):**VAD / MediaPipe / Saliency**。
> **⚠️ 改動**:Saliency 原 Week 3b 走 capacity GPU pool,**2026-06-05 已移出 GPU 改純 CPU pool**
> (onnxruntime CUDAExecutionProvider 與 PyTorch 共卡偶發 hang/OOM,詳見 `lock_design.md` §7);
> VAD 亦從 CPU 單例改 CPU pool(修多影片序列化)。**Gemini 不預載**(雲端 client、無本地權重、未設
> `GEMINI_API_KEY` 會讓啟動失敗)。啟動另印 `StartupReporter` 佈局表(每卡 VRAM / 模型放置 / 各 pool 並行度
> + CPU pool 佈局),逐一推 `MODEL_WARMUP` 事件。`EAGER_MODELS=false` 可關。
...
> **✅ Week 3c 已落地(2026-06-05)**:`WebSocketProgressObserver` + `ProgressHub`(連線中樞:per-job
> bounded replay buffer + per-connection `asyncio.Queue`)把事件橋接到 `WebSocket /ws/progress/{job_id}`。
> 因 `ProgressObserver.on_event` 在 worker thread、WS `send` 在 event loop,中間以 `loop.call_soon_threadsafe`
> 轉交;replay buffer 解「job_id 後端產生、WS 稍晚連線會漏掉開頭事件」競態(attach 時鎖內 snapshot + 註冊原子完成)。
> 連帶把 `/generate` 改 **async job model**(`POST /api/jobs/generate` 立即回 job_id + 背景跑、`GET /api/jobs/{id}`
> 取結果、`JobManager` 落地結果、`JOB_FINISHED`/`JOB_ERROR` 終端事件),保留舊阻塞 `/generate`(expand/contract)。
...
### 7.3 ✅ 已落地的觀測/診斷元件(2026-06-04/05,本機驗;隨 Week 3b 後續一併加入)

實機在共用工作站除錯時補的觀測層,皆為 best-effort、不影響主流程:

- **`StallWatchdog`(`progress/watchdog.py`)**:背景 daemon 每 `WATCHDOG_HEARTBEAT_SEC` 印進行中 stage +
  已執行秒數;心跳停止推進達 `WATCHDOG_FREEZE_DUMP_SEC` → **`faulthandler` 從 C 層 dump 全 thread 堆疊**
  (GIL 被 onnxruntime/CUDA 等 C 擴充凍住時 Python 層抓不到,靠這個兜);另註冊 `SIGUSR1` 手動 dump。
- **`SystemHealthProbe`(`progress/system_health.py`)**:與心跳同行印 `[SysHealth]` 單行(CPU load / RAM /
  **swap in-out 速率** / 各卡 free VRAM),各項超門檻標 `⚠`,一眼分辨「程式卡住」還是「機器被鄰居塞爆」。
- **`ResourceWaitClock`(`model/resource_wait_clock.py`)**:thread-local 累加器,把每個 stage 的耗時拆成
  「**等資源**(borrow 佇列 / GpuGate 預算 / 即時 VRAM 重檢)」與「**真正 compute**」——共用卡被 Qwen 序列化時,
  小模型耗時幾乎全是「等」(log 裡 aes 實算 ~50ms 卻報 91s),拆開後瓶頸一目了然。
- **`StartupReporter`**:啟動印 GPU 佈局表(每卡 VRAM / 模型放置 / 各 pool 並行度)+ CPU pool 佈局。
...
| Drive 資料夾被轉私人 / API key 無權 | 列檔得 401/403 → 該 project `sync_status=paused_auth_error` 暫停同步,其他 project 不受影響;重新分享後手動 sync 喚醒 |
...
- ✅ **Week 2a 已完成**(commit `03b98bb`):ExecutorRegistry + ModelPoolRegistry + Pipeline + HybridScheduler + Builder/Runner + LegacyStage;替換 `director_service.py` 呼叫端
  (注:`StageGroup` 在 2a 時使用;Week 2c 已改用 `StageNode` DAG 取代)
- ✅ **Week 2b 已完成**(2026-06-02):image Stage 拆解(10 Stage + `ImageWork` dataclass)+ Early Rejection 短路;
  semantic 與 saliency/aes/cv/face/exif 同層並行(各只依賴 reject_filter,非獨立成獨立群);
  `USE_LEGACY_IMAGE_PIPELINE` 旗標可 A/B 回退。端到端實機 A/B 待跑
- ✅ **Week 2c 已完成**(2026-06-02):video Stage 拆解(10 Stage + `VideoWork`/`FrameAnalysis` 共用容器)
  + **`StageNode` DAG 編排取代 StageGroup**(`stage_group.py` 已移除);Simple/Complex 兩條 DAG;
  `USE_LEGACY_VIDEO_PIPELINE` 旗標。端到端實機 A/B 待跑

**Week 3**:Layer 1 batch + Layer 4 觀測
- ✅ **Week 3a 已完成**(2026-06-02):MUSIQ/LAION/Whisper/AudioEnv batch scoring 接入 `BatchCollector`;
  `AudioInferenceStage` 全拆成 `VadStage → WhisperStage` + 獨立 `AudioEnvStage`;影片 DAG 重開;
  `MAX_ASSETS_PARALLEL` 動態化(min(asset數,上限),預設 8)。端到端實機 A/B 待跑
- ✅ **Week 3b 已完成**(2026-06-03):`GpuCapacityManager`(`mem_get_info` 掃描 + 優先序 check-before-load 放置 +
  每卡預算)+ `BudgetGate`(VRAM 預算記帳 + Qwen `priority` 反餓死);**所有 GPU 模型接多卡 ModelPool**
  (Qwen 鋪滿可放下的卡、其餘小模型挑最寬鬆卡);eager warm up 預設開;`oom_resilient` 重試;
  `ModelPool.borrow` 即時 VRAM 重檢 + `RESOURCE_WAIT/ACQUIRED` 事件。多 GPU / OOM 實機驗收待跑
- ✅ **Week 3c 已完成**(2026-06-05):`WebSocketProgressObserver` + `ProgressHub`(replay buffer + 佇列 fan-out)
  接 `WebSocket /ws/progress/{job_id}`;`/generate` 改 **async job model**(新增 `POST /api/jobs/generate` 立即回
  job_id + 背景跑、`GET /api/jobs/{id}` 取結果、`JobManager` 落地結果),保留舊阻塞 `/generate`(expand/contract);
  新增 `JOB_FINISHED`/`JOB_ERROR` 事件。結構/邏輯單元與實機 wscat 端到端待跑

**Week 4**:Layer 0 + Layer 5
- ✅ **Week 4a 已完成(2026-06-05,後端;改版為公開資料夾 + Drive API key,實機待驗)**:貼 Drive 公開資料夾 URL →
  建一個 project + polling + 背景觸發 Phase 1。新增 `ingestion_engine/`(`exceptions` / `cloud_storage_adapter`(ABC) /
  `public_drive_api_adapter`(httpx + Drive API v3 + API key)/ `cloud_ingestion_service` / `poller` / `models`)+
  `config/ingestion_config.py` + `backend/api/projects.py` 新端點(`from-drive` / `{name}/sync`);抽出 `run_phase1`;
  `main.py` lifespan 拉起 poller。**設計決策**:徹底移除 rclone 與 OAuth(公開資料夾 + API key,零 per-user token);
  **一個 URL = 一個 project**;持久化用**檔案系統 JSON(無 DB)**,雲端來源與同步狀態(`source`/`drive_folder_id`/
  `phase1_status`/`sync_status`/`last_asset_signature`/`last_synced_at`)折進 `project_meta.json`,poller 掃 project 同步
- ⬜ **Week 4b**:前端 Asset Management UI(未開始)
...
| 檔案 | 改動類型 | 說明 |
|---|---|---|
| `backend/services/director_service.py` | ✅ Week 2a / 3c / 4a 已改 | Week 2a 呼叫端替換為 `PipelineRunner.run(...)`;Week 3c `run_workflow(..., tracker=None)` 透傳 tracker;**Week 4a 抽出 standalone `run_phase1()`**(背景預跑共用),`run_workflow` 內部改呼叫 |
...
| `model/qwen_model_manager.py` | ✅ Week 1 改 / 2026-06-05 換 4B | **bnb 4-bit NF4** + FA2 fallback + device_map 鎖定 + `apply_chat_template` 推理;**模型 8B→4B**(`Qwen3-VL-4B-Instruct`) |
| `model/musiq_model_manager.py` | ✅ Week 1 + Week 3a 改 | Week 1 新增 `score_batch()`;Week 3a 前處理改「保比例+padding」(移除 center-crop)並接入 BatchCollector |
| `model/laion_model_manager.py` | ✅ Week 1 + Week 3a 接入 | Week 1 新增 `score_batch()`;Week 3a 接入 BatchCollector |
| `model/whisper_model_manager.py` | ✅ Week 1 + 3a / 2026-06-05 換後端 | Week 1 `transcribe_batch()`、Week 3a 接 BatchCollector;**改 faster-whisper(CTranslate2)large-v3-turbo**,`transcribe_batch` 改循序(無多檔 forward),補 `info.language` 讓 `spoken_language` 生效 |
...
| `model/audio_env_model_manager.py` | ✅ 2026-06-01 + Week 3a 改 | 修 `inference()` 回傳順序;Week 3a 新增 `classify_environment_batch()` + 抽共用 `_topk_labels` |
| `media_processor/pipeline/progress/` | ✅ Week 1 新增 / 2026-06-04~05 擴充 / 3c | ProgressObserver/Tracker/Event + PrintObserver(events/observer/tracker);**新增** `watchdog.py`(StallWatchdog + faulthandler dump)、`system_health.py`(SystemHealthProbe);**Week 3c** events 加 `JOB_FINISHED`/`JOB_ERROR`、tracker 加 `emit_job_finished`/`emit_job_error` |
| `model/resource_wait_clock.py` | ✅ 2026-06-05 新增 | thread-local 等待累加器:把 stage 耗時拆「等資源」vs「真 compute」 |
| `model/saliency_model_manager.py` | ✅ 2026-06-05 改 | U²-Net **移出 GPU 改純 CPU**(固定 `CPUExecutionProvider`、`self.device='cpu'`);改由 CPU pool 管併發 |
| `model/vad_model_manager.py` | ✅ 2026-06-05 改 | Silero 顯式 `cpu`;**改 CPU pool**(`VAD_POOL_SIZE` 份獨立 instance)修多影片序列化 |
| `config/media_processor_config.py` | ✅ Week 1 改 / 3b / 2026-06-05 補 | batch size / GPU buffer / `QWEN_USE_4BIT_DEFAULT`;3b 各模型 VRAM profile + OOM 重試 + borrow 輪詢;**2026-06-05** `BUDGET_GATE_LOW_PRIORITY_RESERVE_RATIO`(反餓死)+ `INFERENCE_MAX_SHORT_SIDE=720`(4K GIL fix)+ 4B/Whisper VRAM 估值 |
| `config/model_config.py` | ✅ Week 1 改 / 2026-06-05 換型號 | Qwen base id(**改 4B**)+ `QWEN_USE_4BIT` + mediapipe Tasks 常數;**Whisper 改 faster-whisper**(`WHISPER_MODEL_ID=large-v3-turbo` + CT2 compute type / beam / vad_filter 常數) |
| `docs/lock_design.md` | ✅ Week 1 新增 | L1/L2/L3 鎖層級完整設計 |
| `media_processor/abstract_image_processor.py` | ✅ Week 2b(本檔未改) | 邏輯複製到 `pipeline/stages/` 細粒度 Stage;Legacy fallback 仍用本檔 |
| `media_processor/abstract_video_processor.py` | 邏輯沿用(Week 2c) | 內部方法搬到對應 Stage |
| `media_processor/pipeline/`(框架) | ✅ Week 2a 新增 / Week 2c 升級 / 3c | Pipeline / Executor / Scheduler / Builder / Runner 模組樹;Week 2c `StageNode` DAG 取代 StageGroup(`stage_group.py` 已移除,新增 `node.py`);Week 3c `runner.run(..., tracker=None)` 接受注入的 tracker |
| `media_processor/pipeline/stages/`(image) | ✅ Week 2b 新增 | `image_work.py` + 10 image Stage(Decode/Tech/Reject/Saliency/Aes/CV/Face/Exif/Semantic/Assembly)|
| `media_processor/pipeline/stages/`(video) | ✅ Week 2c 新增 / Week 3a 重組 | Week 2c:`video_work.py`/`frame_analysis.py`/`video_frame_utils.py` + 10 video Stage;Week 3a:`audio_inference_stage.py` 移除,新增 `vad_stage.py`/`whisper_stage.py`/`audio_env_stage.py`;影片 DAG 重開 |
| `media_processor/pipeline/batch_collector.py` | ✅ Week 3a 新增 | `BatchSpec`(Value Object)+ `BatchCollector`(Producer–Consumer + Future)+ `BatchCollectorRegistry`(跨 asset 共享單例) |
| `config/pipeline_config.py` | ✅ Week 2a 新增 / 2b / 2c / 3a / 3b / 2026-06-05 | 併發/timeout;2b `USE_LEGACY_IMAGE_PIPELINE`;2c `USE_LEGACY_VIDEO_PIPELINE`;3a 4 個 `*_BATCH_ENABLED`;**2026-06-05** CPU pool 大小(`VAD_POOL_SIZE`/`MEDIAPIPE_POOL_SIZE`/`SALIENCY_POOL_SIZE`)+ Watchdog 常數 + `MAX_ASSETS_PARALLEL` 預設 8→**16** |
| `model/gpu_capacity_manager.py` | ✅ Week 3b 新增 | `mem_get_info` 掃描 + 優先序放置 + 每卡預算 + `apply()` 換 `BudgetGate` |
| `model/gpu_gate.py` | ✅ Week 3b 改 | 新增 `BudgetGate`（VRAM 預算記帳 + `priority` 反餓死 + over-budget 單獨放行）|
| `model/base_model_manager.py` | ✅ Week 3b 改 | gate factory 帶 `device_id`、`INFERENCE_PRIORITY`、`oom_resilient`/`is_cuda_oom` |
| `model/model_pool.py` | ✅ Week 3b 改 | `borrow(observer=)` 即時 VRAM 重檢 + `PoolBorrowObserver` Protocol |
| `media_processor/pipeline/executor/model_pool_registry.py` | ✅ Week 3b 改 | `instance()` 共享 + capacity 整合 + `warm_up` + `borrow_for_batch` |
| `backend/api/progress.py` | ✅ Week 3c 新增 | `ProgressHub`(replay buffer + 佇列 fan-out)+ `WebSocketProgressObserver` + `WebSocket /ws/progress/{job_id}` 端點(replay→串流→哨兵收尾) |
| `backend/services/job_manager.py` | ✅ Week 3c 新增 | `Job`(pydantic)+ `JobManager`(Singleton,執行緒安全 + lazy sweep);async job 結果落地供 GET 取用 |
| `backend/api/director.py` | ✅ Week 3c 改 | 加 `POST /api/jobs/generate`(立即回 job_id + 背景跑)+ `GET /api/jobs/{id}`(取結果)+ `_run_job` 協程;保留舊阻塞 `/generate` |
| `backend/main.py` | ✅ Week 3c / 4a 改 | Week 3c 掛 `progress_router`(不加 `/api` 前綴,路徑即 `/ws/progress/{job_id}`);**Week 4a 改 `lifespan` 啟停 `IngestionPoller`**(4a 改版後移除 `workspaces_router`) |
| `config/app_config.py` | ✅ Week 3c 改 | 加 `PROGRESS_BUFFER_MAXLEN` / `PROGRESS_JOB_RETENTION_SEC` 具名常數 |
| `backend/api/projects.py` | ✅ Week 4a 改(改版) | 加雲端來源端點 `POST /api/projects/from-drive`(貼 Drive 資料夾 URL 建 project + 背景首同步)、`POST /api/projects/{name}/sync`(手動同步);`ProjectMeta` 加 `source`/`phase1_status`/`sync_status`/`last_synced_at` 等 Optional 欄位由 `GET /api/projects` 露出 |
| `ingestion_engine/` | ✅ Week 4a 新增(改版) | Layer 0 攝取模組:`exceptions`(`IngestionError`/`RemoteAccessError`/`RemoteAuthError`)/`models`(pydantic VO + 狀態與 meta-key 具名常數)/`cloud_storage_adapter`(ABC,locator 介面)/`public_drive_api_adapter`(httpx + Drive API v3 + API key:`parse_source`/分頁列檔/`alt=media` 串流下載/錯誤分類)/`cloud_ingestion_service`(Facade:列檔→簽章比對→增量下載→觸發 Phase 1→寫 `project_meta`)/`poller`(async 輪詢,掃所有 project 找 `source=gdrive`) |
| `config/ingestion_config.py` | ✅ Week 4a 新增(改版) | Drive API(`GOOGLE_API_KEY`/`DRIVE_API_*`/`DRIVE_FOLDER_MIMETYPE`)、輪詢、媒體副檔名具名常數;`ENABLE_INGESTION_POLLER` 可關 |
| `backend/services/ingestion_provider.py` | ✅ Week 4a 新增(改版) | Composition Root:組裝 `PublicDriveApiAdapter` / `CloudIngestionService` / `IngestionPoller` 單例 + 注入 `phase1_runner`(重用既有 `director_service`,避免循環 import) |
...
徹底移除 rclone 與 OAuth;模型壓平為 **一個 URL = 一個 project**、同步狀態折進 `project_meta.json`、poller 改掃 project。
§3 加改版 callout、§16 關鍵核心檔案表更新為新檔(`exceptions`/`cloud_storage_adapter`/`public_drive_api_adapter`/
`cloud_ingestion_service`)、§2 架構圖 Layer 0 行、§14 風險表、§15.1 落地紀錄同步更新。詳見 `implementation_roadmap.md` 第 10 章)*
*前版:2026-06-05(Week 4a 完成:Layer 0 Google Drive 攝取**後端**(rclone MVP);新增 `ingestion_engine/` +
`config/ingestion_config.py` + `backend/api/workspaces.py` + `ingestion_provider`,`director_service` 抽出 `run_phase1`、`main.py` lifespan 拉起 poller)*
*前版:2026-06-05(Week 3c 完成:Layer 4 WebSocket 進度推播 + `/generate` 改 **async job model**;
§7.1 補「✅ Week 3c 已落地」(`WebSocketProgressObserver` + `ProgressHub` replay buffer + 執行緒→event loop 橋接)、
§15.1 標記 Week 3c 完成、§16 補錄 `backend/api/progress.py` / `backend/services/job_manager.py` / director(api) / main / app_config)*
*前版:2026-06-05(模型換代與資源歸屬修正:Qwen **8B→4B**、Whisper 改 **faster-whisper turbo**、
Saliency **移出 GPU 改 CPU pool**、VAD 改 CPU pool、BudgetGate 反餓死軟化;§4 加換代修正塊、§5.2 更正 warm_up 歸屬、
§7.3 補觀測元件(StallWatchdog/SystemHealthProbe/ResourceWaitClock)、§16 補相關檔案)*
*前版:2026-06-03(Week 3b 雙 GPU Pool + GpuCapacityManager + BudgetGate 完成;§15.1 標記完成、§16 補錄 Week 3b 接觸檔案;全模型接 pool + Qwen 兩層優先、oom_resilient、borrow 即時 VRAM 重檢等設計決策)*
