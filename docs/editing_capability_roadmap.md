...
> - 🚧 **#2 樣式字幕**進行中：已定案升級為「字幕完全獨立於片段的字幕軌」；v1（per-clip）已在工作樹、v2（解耦＋可拖曳字幕軌）待實作。**完整實作交接見 [`docs/text_overlay_track_handoff.md`](text_overlay_track_handoff.md)**。
> - ⬜ #1 完整版、#3、#4、#5。
...
| 層 | 負責什麼 | 現況 |
|---|---|---|
| **能力層** | renderer 有哪些表現力（濾鏡 / 字幕 / 動畫） | 靜態濾鏡 4 種、字幕寫死單一樣式、無動畫概念 |
...
**相依關係**
- **#1 是地基** → #2 的字幕動畫直接復用它的 keyframe 機制 ⇒ **先 #1 再 #2**。
- **#3 是前提** → #4 的 style 約束要拿 preset 來用 ⇒ **先 #3 再 #4**。
- **#5 規則版獨立**，隨時可插，CP 值高。
...
- **前三步（約 2–3 週）**：成品從「會套效果」變成「**會動、字幕精緻、不會出明顯醜**」，肉眼可見的跳級。
- **#3 / #4**：讓系統**可維護又風格一致**（拔掉 3-way drift、收斂導演自由度）。
- **#5 視覺版**：留到最後當研究題。
