...
Week 2c  ─ Stage 拆解(video)+ DAG 依賴圖編排          ~400 行   ✅ 已完成
   ↓
Week 3a  ─ Dynamic Batching(BatchCollector)            ~300 行   ← 下一步(高風險獨立做)
   ↓
...
*文件最後更新:2026-06-02(Week 2c 影片 Stage 拆解 + DAG 依賴圖編排落地;2a/2b/2c 標記完成,編排原語由 StageGroup 改為 StageNode DAG)*
