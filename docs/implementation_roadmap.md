...
- [Implementation Roadmap:分批落地策略](#implementation-roadmap分批落地策略)
  ...
  - [8. Week 3b:雙 GPU Qwen Pool + GPU Capacity Manager ✅ 已完成(2026-06-03)](#8-week-3b雙-gpu-qwen-pool--gpu-capacity-manager--已完成2026-06-03)
    ...
    - [驗收條件](#驗收條件-4)
    - [風險與緩解](#風險與緩解-2)
    - [再後續(2026-06-05)](#再後續2026-06-05)
  - [9. Week 3c:Layer 4 WebSocket 接前端 ⬜ 未開始](#9-week-3clayer-4-websocket-接前端--未開始)
    - [範圍](#範圍-6)
    - [主要產出](#主要產出-4)
    - [驗收條件](#驗收條件-5)
    - [不做](#不做-4)
  - [10. Week 4a:Layer 0 雲端攝取 ⬜ 未開始](#10-week-4alayer-0-雲端攝取--未開始)
    - [範圍](#範圍-7)
    - [主要產出](#主要產出-5)
    - [驗收條件](#驗收條件-6)
    - [外部依賴(實機驗證)](#外部依賴實機驗證)
...
## 9. Week 3c:Layer 4 WebSocket 接前端 ⬜ 未開始

### 範圍
**與 Week 3a/3b 平行可行**。把 Week 1 已定義的 `ProgressObserver` 介面實作 WebSocket Observer,新增 FastAPI 端點。
> **狀態**:尚未開始(無 `backend/api/progress.py`、backend 無 WebSocket 端點)。ProgressTracker/Observer 介面已就緒(Week 1),
> 觀測側另已落地 `StallWatchdog` / `SystemHealthProbe` / `ResourceWaitClock`(印 log,尚未推 WebSocket)。
...
## 10. Week 4a:Layer 0 雲端攝取 ⬜ 未開始

### 範圍
Google Drive workspace → project 自動偵測。**與 Week 4b 可平行**。
> **狀態**:尚未開始(無 `ingestion_engine/`、`backend/api/workspaces.py`)。
...
*文件最後更新:2026-06-05(模型換代 + 共用機實機修正:Qwen **8B→4B**、Whisper 改 **faster-whisper turbo**(§3 加換代塊)、
