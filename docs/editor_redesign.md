...
**目標版面（三欄）**：左側較窄的版本快照清單（固定 260px）、中央加寬的 9:16 預覽（吃滿剩餘空間、隨螢幕放大）、右側 Inspector（固定 360px，選取片段後調屬性）、底部全寬真實時間軸；AI 對話為可收合的側邊 copilot 抽屜（工具列「💬 AI」開關）。左右用固定寬度而非各 1/3：清單與 Inspector 維持可讀，預覽自動取得最多空間。
