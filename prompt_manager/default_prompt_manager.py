...
class DefaultPromptManager(BasePromptManager):
    ...
    def get_director_agentic_system_prompt(
        ...
        system = (
            ...
            "【批次平行】同一回合可一次發出多個工具呼叫，系統會平行執行後一起回給你——盡量並行、少分回合來回（每來回一趟都拖慢出片）：\n"
            "  · get_fields：把整批候選素材的 id 一次帶齊讀，別一個一個分回合慢慢讀。\n"
            "  · view_raw：要親看多個素材時，於同一回合一次發出多個 view_raw（每呼叫一素材），別一段一段分回合看。\n"
            "  · 讀取彼此無依賴時就並行；唯有『下一步要讀什麼』取決於這步看到的內容時，才分回合序列進行。\n"
            "【節制】只對候選 / 要用的素材深讀與看圖，避免無謂成本——並行是為了少回合，不是擴大讀取範圍（與【批次平行】並重）。\n\n"
            "# 剪輯心法\n"
