...
class DefaultPromptManager(BasePromptManager):
    ...
    def get_media_analysis_prompt(self) -> str:
        return (
            ...
            "3. 請依照定義分析以下語意屬性。\n"
            "4. 請框出畫面中『最主要的單一主體』(最重要的人或物)的外接矩形，作為 subject_bbox：\n"
            "   - 格式為 [x1, y1, x2, y2]，數值是相對影像寬高、正規化到 0–1000 的整數(x1,y1 為左上角；x2,y2 為右下角)。\n"
            "   - 只框最主要的單一主體，切勿把整個畫面或多個物體一起框進去。\n"
            "   - 若畫面沒有明確主體(如純風景、抽象畫面)，請將 subject_bbox 設為 null。\n"
    ...
    def get_timecoded_action_index_prompt(self) -> str:
        return (
            ...
            "6. 在每個區塊中，請框出『該 key_timestamp 當下、畫面最主要的單一主體』的外接矩形，作為 subject_bbox：\n"
            "   - 格式為 [ymin, xmin, ymax, xmax]，數值正規化到 0–1000(ymin,xmin 為左上角；ymax,xmax 為右下角)。\n"
            "   - 只框最主要的單一主體，切勿框住整個畫面或多個物體。若該區段無明確主體，請填 null。\n"
            ...
            "      \"key_timestamp\": 3.2,\n"
            "      \"subject_bbox\": [ymin, xmin, ymax, xmax],\n"
