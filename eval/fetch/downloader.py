"""影片/縮圖下載與快取索引（冪等、可重複執行）。

以 ``cache_key``（platform:video_id）為鍵維護一份索引；已下載且檔案仍在者跳過，達成「已抓過不重抓」。
...
class FetchIndex:
    """已下載快取索引：cache_key → {video, thumbnail} 本機路徑。"""
    ...
    def put(self, cache_key: str, video_path: str, thumbnail_path: str | None) -> None:
        """寫入某鍵的本機路徑記錄。"""
        self._entries[cache_key] = {"video": video_path, "thumbnail": thumbnail_path or ""}
...
class ClipDownloader:
    ...
    def ensure_downloaded(
        ...
        *,
        ...
        """確保候選的影片（與縮圖）已在本機；回傳補上本機路徑的候選。

        若索引已記錄且檔案仍存在 → 直接沿用（不重抓）；否則下載並更新索引。
        """
        cached = index.get(candidate.cache_key)
        if cached and Path(cached.get("video", "")).is_file():
            # 快取命中：沿用既有檔案，補回路徑
            thumb = cached.get("thumbnail") or None
            return candidate.model_copy(update={"local_path": cached["video"], "thumbnail_path": thumb})

        # 下載影片
        video_name = self._build_filename(candidate, candidate.download_url, DEFAULT_VIDEO_EXT)
        video_path = candidates_dir / video_name
        logger.debug("下載影片 %s → %s", candidate.cache_key, video_path)
        self._http.download(candidate.download_url, video_path)
        ...
        index.put(candidate.cache_key, str(video_path), str(thumbnail_path) if thumbnail_path else None)
    ...
    def _build_filename(candidate: ClipCandidate, url: str, default_ext: str) -> str:
        """以 platform_videoid + URL 副檔名（或預設）組檔名。"""
        suffix = Path(urlparse(url).path).suffix
        ext = suffix if suffix else default_ext
        return f"{candidate.source_platform.value}_{candidate.video_id}{ext}"
